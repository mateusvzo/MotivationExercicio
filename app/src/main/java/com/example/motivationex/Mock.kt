...
class Mock {
    ...
    fun filterPhrase(id: Int): String {
        var filteredPhrase = mListPhrase.filter { it.categoryId == id || id == all }
        var index = Random.nextInt(filteredPhrase.size)
        return filteredPhrase[index].description
    }
}